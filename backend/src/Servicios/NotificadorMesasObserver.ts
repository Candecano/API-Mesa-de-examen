...
export class NotificadorMesas {
  ...
  detach(observer: Observer): void {
    ...
    console.log("🗑️ Observador eliminado. Total:", this.observers.length);
  }

  // Notificar a todos los observadores de una nueva mesa
  notify(mesa: MesaInfo): void {
    if (this.observers.length === 0) {
      console.warn("⚠️ No hay observadores registrados para notificar.");
      return;
    }
    ...
    this.observers.forEach((observer, index) => {
      console.log(`➡️ Notificando observador ${index + 1}`);
